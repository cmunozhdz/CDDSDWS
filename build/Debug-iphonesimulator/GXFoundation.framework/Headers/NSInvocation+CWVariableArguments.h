 ...
 * @param ... a list of arguments to send to the method when invoking, primitive types must be wrapped inside an NSValue object, ie. @(PRIMITIVE_VALUE).
 ...
 *
 * @param retainArguments YES if object arguments should be retained.
 * @param arguments a variable arguments list with arguments to send to the method when invoking, primitive types must be wrapped inside an NSValue object, ie. @(PRIMITIVE_VALUE).
 * @result a prepared invocation object.
 */
+(NSInvocation*)invocationForInstancesOfClass:(Class)aClass
                                 withSelector:(SEL)selector
 ...
 *
 * @discussion You should only read the return value if you have waited until the
 *           invocation is done.
 */
...
+ (id)gxObjectValueFrom:(const void *)value objCType:(const char *)type;

- (id)getReturnGxObjectValue;

- (id)getArgumentGxObjectValueAtIndex:(NSInteger)idx;
